...
// Component originally created by Dominik Koch
// Modified for ReactNode items
...
function OrbitItem({ item, index, totalItems, path, itemSize, rotation, progress, fill }: OrbitItemProps) {
  ...
  return (
    ...
    >
      <div style={{ transform: `rotate(${-rotation}deg)` }} className="w-full h-full">{item}</div>
    </motion.div>
  );
}
...
export default function OrbitImages({
  ...
  useEffect(() => {
    ...
    const target = current + (direction === 'reverse' ? -100 : 100);
