...
export function AuthModal({ isOpen, onClose, onSuccess, isBuying = false }: AuthModalProps) {
  ...
  useEffect(() => {
    if (isOpen && googleLoaded && googleButtonRef.current && window.google?.accounts?.id) {
      ...
      try {
        ...
        // Use standard Google button to securely trigger FedCM Button Mode
  ...
  }, [isOpen, googleLoaded, isBuying, handleAuthSuccess]);

  async function handleGoogleSignInFallback() {
    setBusy(true);
    setError(null);

    // Fallback for non-Chromium browsers without FedCM support (e.g. Safari, Firefox)
    try {
      const auth = firebaseAuth();
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm animate-in fade-in duration-150">
      ...
      <div className="relative w-full max-w-md rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl p-6 sm:p-8 z-10 animate-in zoom-in-95 duration-150">
        ...
        <div className="flex flex-col items-center justify-center space-y-3">
          {busy ? (
            <div className="w-full flex items-center justify-center gap-3 px-6 py-3.5 rounded-full bg-slate-50 dark:bg-slate-800/90 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white font-medium text-sm transition-all shadow-sm">
            ...
            <div className="w-full flex flex-col items-center gap-3">
              ...
              {!googleLoaded && (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-3 px-6 py-3.5 rounded-full bg-white dark:bg-slate-800/90 border border-slate-300 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-900 dark:text-white font-medium text-sm transition-all shadow-sm cursor-pointer"
                >
