...
export function AuthModal({ isOpen, onClose, onSuccess, isBuying = false }: AuthModalProps) {
  ...
  // Fallback to standard popup if GIS is not available
  async function handleFallbackGoogleSignIn() {
    setBusy(true);
    setError(null);
    try {
    ...
    const tryRender = () => {
      if (window.google?.accounts?.id && googleButtonRef.current) {
        try {
          ...
          // Adapt GIS button theme to dark/light mode
  ...
  }, [isOpen, isBuying, clientId, resolvedTheme, handleAuthSuccess]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm animate-in fade-in duration-150">
      ...
      <div className="relative w-full max-w-md rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl p-6 sm:p-8 z-10 animate-in zoom-in-95 duration-150">
        ...
        {/* Google Sign-In Button with GIS theme adaptation */}
        <div className="flex flex-col items-center justify-center space-y-3 min-h-[50px]">
          {busy ? (
            <div className="w-full flex items-center justify-center gap-3 px-6 py-3.5 rounded-full bg-slate-50 dark:bg-slate-800/90 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white font-medium text-sm shadow-sm opacity-60">
            ...
            <button
              ...
              disabled={busy}
              className="w-full flex items-center justify-center gap-3 px-6 py-3.5 rounded-full bg-slate-50 dark:bg-slate-800/90 border border-slate-300 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-900 dark:text-white font-medium text-sm transition-all shadow-sm cursor-pointer disabled:opacity-50"
            >
