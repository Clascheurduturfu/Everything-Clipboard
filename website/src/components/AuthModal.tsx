...
export function AuthModal({ isOpen, onClose, onSuccess, isBuying = false }: AuthModalProps) {
  ...
  async function handleGoogleSignIn() {
    ...
    // 1. Try FedCM Button Mode (Active Mode) for native Chrome modal dialog (like Samsung)
    if (typeof navigator !== "undefined" && "credentials" in navigator && clientId) {
      try {
        ...
        // If the user dismissed or cancelled the native FedCM modal dialog, exit cleanly
    ...
    // 2. Fallback to standard Firebase Popup for Safari, Firefox, or unsupported environments
    try {
      const auth = firebaseAuth();
